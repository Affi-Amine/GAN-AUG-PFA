...
DATASET_SUBDIR_DEFAULT = "Onera Satellite Change Detection Dataset" # User specified dataset subdir
SYNTHETIC_DATA_DIR_DEFAULT = "synthetic_data"
CHECKPOINT_DIR_DEFAULT = "siamese_checkpoints"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
BATCH_SIZE_DEFAULT = 4 # Adjusted default
...
USE_SYNTHETIC_DEFAULT = False # Default to not using synthetic data
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for batch in progress_bar:
        if batch is None: continue # Skip if batch is None (due to collate_fn)
...
def main(args):
    ...
    # Datasets and Dataloaders using the factory function
    ...
    val_dataset = create_change_detection_dataset(
        ...
        mode="val",
        target_size=target_size,
        use_synthetic=False # Never use synthetic for validation
    )
    ...
    if len(val_dataset) == 0:
        ...
        # Continue training, but validation metrics won't be useful

    # Use collate_fn to handle None samples from dataset loading errors
